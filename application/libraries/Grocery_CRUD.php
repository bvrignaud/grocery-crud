...
class grocery_CRUD_Field_Types
{
	...
	protected function get_field_input($field_info, $value = null)
	{
	    if (isset($field_info->type)
	            && $field_info->crud_type == 'readonly' && in_array($field_info->type, array('date', 'datetime'))) {
	...
	{
	    if (isset($field_info->type)
	            && in_array($field_info->type, array('date', 'datetime'))
	            && $field_info->crud_type == 'readonly') {
	...
	protected function get_read_input_fields($field_values = null)
	{
		...
		foreach ($read_fields as $field) {
			if (empty($this->change_field_type)
    			...
    			if (isset($types[$field->field_name]->type)
    			        && in_array($types[$field->field_name]->type, array('date', 'datetime'))) {
